    ...
    map.forEachFeatureAtPixel(pixel, function (feature, layer) {
      ...
      if (clickable && feature && (!lagNavn.includes("Midlertidig") && !lagNavn.includes("midlertidig"))) {
        ...
        if(lagNavn.includes("Ikon") || lagNavn.includes("ikon")){
          ...
          // var eksisterer = featureEksistererIFeatureSelectionList(feature);
          var eksisterer = featureEksistererIFeatureClickSelection(feature);
          ...
          if(!eksisterer && currentItems < maxItems){
            currentItems++;
            fullTekst += lagFeatureSelectionPopupTekst(feature, layer, koordinater);
            // settStilerOgLeggTilFeatureISelectionList(feature, layer);
      ...
      //   // console.log("featureClickSelectionList.length: " + featureClickSelectionList.length);
      //   // console.log(featureInfoData);

      //   if(featureClickSelectionList.length == 1 && featureInfoData.length > 0){
      //     console.log(featureClickSelectionList);
      //     console.log(featureInfoData);
      //     // console.log("Bare en tur er trykket på, viser info");
      //     visFeatureInfoSide(featureInfoData[0], featureInfoData[1], featureInfoData[2], false);
      //     // Skjule popup når det uansett bare er en tur.
      //     skjulPopupContainer();
      //   } else {
      //     // console.log("Flere enn en tur trykket på");
      //   }
...
function lagFeatureSelectionPopupTekst(feature, layer, koordinater){
  var endeligeNavn = hentFeatureNavnMedBackup(feature);
  ...
  const onclickFeatureIPopup = "featureKlikkIPopup(" + uid + ",'" + lagNavn + "');";
...
function featureKlikkIPopup(featureUID, lagNavn){
  // console.log("featureKlikkIPopup klikket! featureUID: " + featureUID + ", lagNavn: " + lagNavn);
  ...
  if(kartlagFeature != null){
    ...
    if(KartKjorer && (kartlag.get("ikonLag"))){
      ...
      visFeatureInfoSide(kartlagFeature, kartlagFeatureNavn, kartlag, false);

      // selectRute(kartlagFeature, kartlag);

    } else {
      console.log("Kartlaget er ikke et turkartlag!");
    }

  } else {
    console.log("Fant ikke tur feature med UID: " + featureUID);
  }
  
}

// Tømmer selectionListe og uthever en enkelt rute.
function selectRute(feature, kartlag){
  nullstillFeatureClickSelection();
